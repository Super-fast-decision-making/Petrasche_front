...
function tagToLink(str){
    ...
    var txt = newText.replace(/#[^#\s,;]+/gm, function(tag) {
        console.log("!!!!!!!!");
        return '<a href="#" onclick="hashtag_search()" id="hashtag_search" class="hashtag">' + tag + '</a>'
    });
    txt = txt.replace(/\n/g, "<br>")
    return txt;
}
...
async function hashtag_search() {
    const words_for_search = document.getElementById("hashtag_search").value;
